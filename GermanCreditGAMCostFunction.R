...
cost <- function(x){
  ...
  for(ind in 1:round(x[1])){ #3
    ...
    for (pCut in c(0.05, 0.06, 0.07, 0.08, 0.09, 0.1, 0.11, 0.12, 0.13, 0.14, 0.15, 0.16, 0.17, 0.18, 0.19, 0.2, 0.21, 0.22, 0.23, 0.24,
      ...
      reducedGam.object <- gam(V25 ~ V1 + V2 + V3 + V5 + V6 + V11 + V16 + V18 + V19, data=train, family=errDist_LinkFunc)
      ...
      pDmaj<-sum(respDmaj$V25==respDmaj$prevs)/nrow(respDmaj)
      
      pDmin<-sum(respDmin$V25==respDmin$prevs)/nrow(respDmin)
  ...
  test<-subComplete[test.idx,c("V1", "V2", "V3", "V5", "V6", "V11", "V16", "V18", "V19", "V25")]
  ...
  pDmaj<-sum(respMaj$V25==respMaj$prevs)/nrow(respMaj)
  
  pDmin<-sum(respMin$V25==respMin$prevs)/nrow(respMin)
  
  allResp<-rbind(respMin,respMaj)
  
  cm<-confusionMatrix(data=as.factor(allResp$V25), as.factor(allResp$prevs), mode="everything")
  ...
  f9<-auc(as.numeric(allResp$prevs),as.numeric(allResp$V25))
